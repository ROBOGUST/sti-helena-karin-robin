package sti.helena.karin.robin.domain;

import java.util.ArrayList;
import java.util.Scanner;

public class Vault {

    Student student1= new Student("Adam", "Svensson", "19730412-4433", 1234);
    Student student2= new Student("Klara", "Olsson", "19920327-3445", 1456);
    Student student3= new Student("Sven", "Svensson", "19730232-4556", 1456);

    Course kurs1= new Course();
    Course kurs2= new Course();
    Course kurs3= new Course();

    Teacher teacher1 = new Teacher("Gurran", "Karlsson", 195604302354L);
    Teacher teacher2 = new Teacher("Gittan", "Gittanson", 197303126543L );






    ArrayList<String> Students = new ArrayList<>();


}
